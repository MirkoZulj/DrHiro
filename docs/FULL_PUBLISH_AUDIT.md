...
|---|---|---|
| `apps/api/src/drhiro_api/services/intelligent_food_search.py:99-102` | **Hardcoded** `sshpass -p <redacted-secret>` + `mirko@100.75.194.51` (Pi SSH). Real password + host. | **never-copy as-is** → must become env-driven or replaced (this is the deprecated Pi-SSH food path). |
