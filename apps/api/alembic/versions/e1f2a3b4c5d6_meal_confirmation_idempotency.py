...
by f1a2b3c4d5e6 and is declared on the ORM ``Meal``. This migration only adds
the unique index. Because the column is **nullable** (legacy rows carry
``NULL``), the index is ``PARTIAL`` (``WHERE source_operation_id IS NOT NULL``)
so multiple legacy NULL rows remain allowed -- a plain unique constraint would
reject every NULL row beyond the first.
